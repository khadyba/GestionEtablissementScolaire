    ...
    public function update(User $user, Cours $cours)
{
    ...
    if ($user->role_id !== 1 || $cours->user_id !== $user->id) {
        // Retourner une réponse avec un message d'erreur personnalisé et rediriger vers la liste des cours
        return redirect()->route('professeurs.cours.list')->with('error', 'Vous n\'avez pas l\'autorisation de modifier ce cours.');
    }

    // Si la condition est valide, continuez le processus de mise à jour
    // Logique de mise à jour ici (par exemple, l'édition du profil ou autre action)
    // Si tu as des actions supplémentaires, tu peux les inclure ici.

    // Exemple : retourner la vue de modification du cours (ou autre logique selon ton besoin)
